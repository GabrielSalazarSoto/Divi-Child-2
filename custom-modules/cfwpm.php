			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
				...
				<?php
					if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_categories || 'on' === $show_comments ) {
                        ...
                        if (in_category(array('24','34'))){$salida=get_the_excerpt();}elseif (in_category(array('25','32','49','51'))){$salida=esc_html("News Alert ".get_the_excerpt());}else{$salida=esc_html( $categories[0]->name );}
						...
						printf( '<p class="post-meta">%1$s %2$s %3$s %4$s %5$s %6$s %7$s</p>',
							...
							(
								'on' === $show_categories
									? $salida
                                //? $salida."<br><a href='".get_edit_post_link()."' target=blank class=post-edit-link>Editar</a>"
									: ''
							),
							(
